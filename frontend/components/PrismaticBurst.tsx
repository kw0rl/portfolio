...
const PrismaticBurst = ({
  ...
  colors = ['#00ffff', '#0080ff', '#0040ff', '#8000ff'],
  distort = 2,
  paused = false,
  offset = { x: 0, y: 0 },
  hoverDampness = 0.1,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not supported, PrismaticBurst will not render:', error);
  ...
  useEffect(() => {
    ...
    // Compute effective values here (same logic as used when creating the renderer)
    const effectiveIntensity = isMobile ? Math.min(intensity * 0.5, 0.8) : intensity;
    const effectiveSpeed = isMobile ? Math.min(speed * 0.7, 0.3) : speed;
    const effectiveRayCount = isMobile ? Math.min(rayCount, 6) : rayCount;

    program.uniforms.uIntensity.value = effectiveIntensity ?? 1;
    program.uniforms.uSpeed.value = effectiveSpeed ?? 1;
    ...
    program.uniforms.uRayCount.value = Math.max(0, Math.floor(effectiveRayCount ?? 0));
  ...
  }, [intensity, speed, animationType, colors, distort, offset, rayCount, isMobile]);

  return (
    <div className="w-full h-full relative overflow-hidden" ref={containerRef}>
      {!webglSupported && (
        <div className="w-full h-full bg-gradient-to-br from-pink-300/10 to-purple-400/10 flex items-center justify-center">
          <div className="text-white/50 text-center">
            <p className="text-sm">WebGL not supported</p>
            <p className="text-xs">Using fallback background</p>
          </div>
        </div>
      )}
    </div>
